...
Function Display-Files-And-Folders {
    Get-ChildItem -Path "C:\Users\elava\Downloads\Powershell\Monitoring_Files" -Recurse
}
...
Function CentralLog {
    ...
    $centralLogFile = "C:\Users\elava\Downloads\Powershell\New folder\CentralLog.txt"

    # Check if the baseline.txt file exists before clearing its content
    $baselineFilePath = ".\baseline.txt"
    if (Test-Path $baselineFilePath) {
        Clear-Content -Path $baselineFilePath
    } else {
        Write-Host "Baseline file not found. Skipping clear operation." -ForegroundColor Yellow
    }
    ...
    $files = Get-ChildItem -Path "C:\Users\elava\Downloads\Powershell\Monitoring_Files" -File -Recurse
